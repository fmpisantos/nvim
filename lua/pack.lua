...
local function setup_plugin(setup_config, plugin_name, dependency_chain)
    ...
    if setup_config.deps then
        ...
        for _, dependency in ipairs(list) do
            if not setup_completed[dependency] then
    ...
    if type(setup_config.setup) == "function" and not setup_completed[plugin_name] then
        ...
        if events then
            ...
            vim.api.nvim_create_autocmd(events, {
                callback = function()
                    ...
                    if not setup_completed[plugin_name] then
                        vim.print(plugin_name)
                        setup_config.setup()
                        setup_completed[plugin_name] = true
                    else
                        vim.print("already: " .. plugin_name)
                    end
                end,
            })
        else
