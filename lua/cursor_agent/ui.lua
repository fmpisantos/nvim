...
-- UI Module
-- =============================================================================
-- Windows, buffers and titles for cursor_agent.
...
function M.create_response_split(name, clear)
    ...
    local autocmd_group = vim.api.nvim_create_augroup("CursorAgent_NoSave_" .. state.response_buf, { clear = true })
    ...
    vim.wo[state.response_win].wrap = state.user_config.response_buffer.wrap
    vim.b[state.response_buf].cursor_agent_session_id = state.current_session_id
    ...
    end, { buffer = state.response_buf, noremap = true, silent = true, desc = "Close CursorAgent response" })
...
function M.toggle_response_buffer()
    ...
    if not state.response_buf or not vim.api.nvim_buf_is_valid(state.response_buf) then
        vim.notify("No CursorAgent chat active. Use <leader>ca to start one.", vim.log.levels.INFO)
        return
    end
...
function M.get_window_title(content, session_id)
    ...
    local title = " CursorAgent [" .. mode .. "] [" .. config.get_model_display() .. "]"
    if session_id then
...
function M.setup_auto_reload()
    ...
    local augroup = vim.api.nvim_create_augroup("CursorAgentAutoReload", { clear = true })
