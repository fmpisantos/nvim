...
-- =============================================================================
-- claude_code - Main Module
-- =============================================================================
-- A Neovim integration for the `claude` CLI (Claude Code), mirroring the
-- opencode.nvim workflow: a floating prompt window, a streaming response
-- buffer, model and mode selection, and session continuation.

local config = require("claude_code.config")
local utils = require("claude_code.utils")
local ui = require("claude_code.ui")
local sessionmod = require("claude_code.session")
local runner = require("claude_code.runner")
...
function M.Open(initial_prompt, filetype, source_file, session_id_to_continue)
    ...
    if not state.prompt_buf or not vim.api.nvim_buf_is_valid(state.prompt_buf) then
        local existing = vim.fn.bufnr("claudecode://prompt")
    ...
    if reuse_existing_buffer and state.prompt_buf and vim.api.nvim_buf_is_valid(state.prompt_buf) then
        ...
        local name_buf = vim.fn.bufnr("claudecode://prompt")
        if name_buf == -1 or not vim.api.nvim_buf_is_valid(name_buf) then
            vim.api.nvim_buf_set_name(buf, "claudecode://prompt")
        else
            vim.api.nvim_buf_set_name(buf, "claudecode://prompt-" .. buf)
        end
    end

    state.prompt_buf = buf
    state.prompt_win = win
    vim.b[buf].claude_code_source_file = source_file
    ...
    local augroup_name = "ClaudeCodePrompt_" .. buf
...
function M.SelectModel()
    ...
    -- `claude` has no `models` subcommand, so the list is static (see config.MODELS).
    local models = vim.deepcopy(config.MODELS)

    pickers.new({}, {
        prompt_title = "Select ClaudeCode Model",
        ...
        attach_mappings = function(prompt_bufnr, _)
            actions.select_default:replace(function()
                ...
                if selection and selection.value then
                    ...
                    config.save_config()
                    vim.notify("ClaudeCode model set to: " .. config.get_model_display(), vim.log.levels.INFO)
                end
            end)
            return true
        end,
...
---@param mode? string "agent" | "plan" | "ask" | "quick" (cycles when nil)
function M.SetMode(mode)
    ...
    if config.set_mode(mode) then
        config.save_config()
        vim.notify("ClaudeCode mode set to: " .. mode, vim.log.levels.INFO)
    end
end
...
local function open_session_picker(callback)
    ...
    pickers.new({}, {
        prompt_title = "ClaudeCode Sessions",
        finder = finders.new_table({
            results = entries,
            entry_maker = function(entry)
                ...
                if selection then
                    ...
                    if callback then
                        ...
                        if entry.id then
                            ...
                            local buf, _ = ui.create_response_split("ClaudeCode Response", true)
                            vim.api.nvim_buf_set_lines(buf, 0, -1, false, vim.split(content, "\n", { plain = true }))
                            vim.b[buf].claude_code_session_id = entry.id
...
function M.setup(opts)
    ...
    local commands = require("claude_code.commands")
    ...
    vim.api.nvim_create_autocmd("QuitPre", {
        callback = function()
            ...
            local prompt_buf = vim.fn.bufnr("claudecode://prompt")
