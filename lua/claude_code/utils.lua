...
-- Utilities Module
-- =============================================================================
-- Shared helpers for claude_code.
...
--- Recognizes: agent, plan, ask, quick as leading words and #agent, #plan,
--- #ask, #quick tags.
---@param content string
...
--- inlined contents of every referenced file. The CLI runs with `--tools ""`,
--- so these contents are the only context the model gets.
---@param prompt string
...
--- Build the claude command array.
--- The prompt is NOT part of the command: it is written to the process's stdin.
--- `--tools` is variadic in the CLI and would swallow a trailing positional
--- prompt argument, and stdin also avoids argv length limits when quick mode
--- inlines whole files.
---@param opts table { model?: string, mode?: string, resume_id?: string }
---@return table cmd
function M.build_claude_cmd(opts)
    local user_config = config.state.user_config
    local cmd = { "claude", "-p", "--output-format", "stream-json", "--verbose" }

    -- stream-json only emits incremental deltas when partial messages are on.
    if user_config.stream_partial then
        table.insert(cmd, "--include-partial-messages")
    end

    -- Model (alias such as "opus"/"sonnet", or a pinned model id)
    ...
    -- Mode -> permission mode / tool allowlist
    ...
    elseif mode == "ask" then
        ...
        table.insert(cmd, config.ASK_MODE_TOOLS)
    elseif mode == "quick" then
        -- Empty string disables every built-in tool.
        table.insert(cmd, "--tools")
        table.insert(cmd, "")
    else
        table.insert(cmd, "--permission-mode")
        table.insert(cmd, user_config.force and "bypassPermissions" or "acceptEdits")
    end
